...
public class BookLessonFragment extends Fragment {
            ...
            db.collection("users").document(firebaseUser.getUid()).get().addOnSuccessListener(documentSnapshot -> {
                if (documentSnapshot.exists()) {
                    ...
                    if (currentUser != null) {
                        ...
                        boolean hasReservation = currentUser.getReservations().stream().anyMatch(reservation -> reservation.getDateTime().equals(formatDateTime(selectedDateTime.getTime())));
    ...
    private boolean isTeacherAvailable(User teacher, Date selectedDateTime) {
        ...
        return teacher.getAvailability() != null && teacher.getAvailability().containsKey(formattedDate) && teacher.getAvailability().get(formattedDate).contains(new SimpleDateFormat("HH", Locale.getDefault()).format(selectedDateTime));
    }
    ...
    private void bookLessonWithTeacher(User teacher) {
        ...
        db.runTransaction(transaction -> {
            ...
            if (court != null && user != null && updatedTeacher != null) {
                ...
                Reservation newReservation = new Reservation(reservationId, court.getId(), formatDateTime(selectedDateTime.getTime()), true, // isLesson set to true
                        userId);
